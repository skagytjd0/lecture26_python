...
if __name__ == '__main__':
    dao = MemberDAO()
    print(dao.is_exist('skagytjd'))

    member = Member('skagytjd', '1234', '남효성')
    dao.insert_member(member)
    member = Member('ska', '1234', '효')
    dao.insert_member(member)
    print(dao.get_member_info('skagytjd'))
    print(dao.get_member_info('ska'))

    members = dao.get_all_members()
    for member in members:
        print(member)

    member = dao.get_member_info('skagytjd')
    if member:
        member.set_password('1111')
        dao.update_member_info('skagytjd', member)
    
    members = dao.get_all_members()
    for member in members:
        print(member)

    dao.remove_member('skagytjd')
    members = dao.get_all_members()
    for member in members:
        print(member)